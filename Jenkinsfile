pipeline {
    ...
    stages {
        stage('Checkout git') {
            steps {
               git branch: 'sonar', url: 'https://github.com/phani-rudra9/java-app'
            }
        }
        ...
        stage('Building Docker Image'){
            steps{
                sh '''
                sudo docker build -t phani/devsecops-demo:$BUILD_NUMBER .
                sudo docker images
                '''
            }
        }
        stage('Image Scanning Trivy'){
            steps{
               sh 'sudo trivy image phani/devsecops-demo:$BUILD_NUMBER > $WORKSPACE/trivy-image-scan/trivy-image-scan-$BUILD_NUMBER.txt'
        ...
        stage('Pushing Docker Image into Docker Hub'){
            steps{
                ...
                sh '''
                sudo docker login -u phani -p $DOCKERHUB_PASSWORD
                sudo docker push phani/devsecops-demo:$BUILD_NUMBER
                '''
               }
            }
        }        
        ...
        stage('Cleaning up DockerImage'){
            steps{
                sh 'sudo docker rmi phani/devsecops-demo:$BUILD_NUMBER'
            }
        }
    }
  }
